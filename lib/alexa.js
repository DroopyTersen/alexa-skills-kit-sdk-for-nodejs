...
function ResponseBuilder(self) {
    ...
    return (function () {
        return {
            ...
            'displayTemplate': function(template, hint) {
                if (!template) return this;

                responseObject.response.directives = responseObject.response.directives || [];
                if (supportsDisplay(self)) {
                    responseObject.response.directives.push(createDisplayDirective(template));
                }
                if (hint) {
                    responseObject.response.directives.push(createHintDirective(hint))
                }
                return this;
            },
            'confirmIntent': function(speech, intent) {
                var outputSpeech = createSSMLSpeechObject(speech);
                var response = {
                    outputSpeech,
                    reprompt: { outputSpeech },
                    shouldEndSession: false,
                    directives: responseObject.response.directives || [],
                }   
                var directive = {
                    type: "Dialog.ConfirmIntent"
                } 
                response.directives.push(directive)
                responseObject.response = Object.assign({}, responseObject.response, response)
                return this;                 
            },
            'elicitSlot': function(slotName, prompt, updatedIntent) {
                var outputSpeech = createSSMLSpeechObject(prompt);
                var response = {
                    directives: responseObject.response.directives || [],
                    outputSpeech,
                    reprompt: { outputSpeech },
            ...
            'delegate': function(updatedIntent) {
                responseObject.response.directives = responseObject.response.directives || [];
                responseObject.response.shouldEndSession = false;
                var directive = { type: "Dialog.Delegate" };
                if (updatedIntent) directive.updatedIntent = updatedIntent;
                responseObject.response.directives.push(directive);
                return this;
            },
...
function createDisplayDirective(template) {
    return {
...
function supportsDisplay(skill) {
    try {
        // Simulator doesn't pass context otherwise try to check for display
        return skill._event.context.System.device.supportedInterfaces.Display;
    } catch (ex) {
        return false;
    }
}

function createHintDirective(hint) {
    // If it's not a string assume it is already hint directive object { type, text }
